...
class Consumable extends SnipeModel
{
    ...
    {
        ['local' => $localFields, 'relations' => $relationFields] = self::getSearchableFields();

        return $query->where(function ($q) use ($searchTerm, $localFields, $relationFields) {
            foreach ($localFields as $field) {
                $q->orWhere("consumables.$field", 'LIKE', '%' . $searchTerm . '%');
            }

            foreach ($relationFields as $relation => $fields) {
                foreach ($fields as $field) {
                    $q->orWhereHas($relation, function ($relQuery) use ($field, $searchTerm) {
                        $relQuery->where($field, 'LIKE', '%' . $searchTerm . '%');
                    });
                }
            }
        });
    ...
    public static function applyFilters($query, $filters)
    {
        ...
        foreach ((array) $filters as $key => $value) {
            ...
            if (in_array($key, $localFields)) {
                $query->where("consumables.$key", 'LIKE', '%' . $value . '%');
                continue;
            }
